...
const withCheckout = graphql(checkout, {
  ...
  options({ checkout, checkoutId, handleCheckoutCreation, isCheckoutLoading }) {
    const hoursPastCreation =
      checkout && dayjs().diff(dayjs(checkout.createdAt), 'hours')
    const withShippingRates =
      checkout && checkout.requiresShipping && checkout.shippingAddress
